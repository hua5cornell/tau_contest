...
In this contest, we implement the slew target method presented in~\cite{Held:Gate} because it can address slew violations and it is highly parallelizable. It also avoids large number of incremental timing updates. Since the paper focuses on fixing setup time violations, we generalize the method a little bit such that it also addresses hold time violations.
...
In~\cite{Held:Gate}, each gate output pin is assigned a slew target to guide the gate sizing process. Slew target method is based on the assumption that the delay is a monotonic function of the input slew. This assumption generally holds in practical design as can be seen in the cell library file, but there might be small fluctuations on delay/output slew vs input slew where the monotone property fails. Empirically, we do not have to worry about this because output slew and delay are weak functions of input slew, and the small fluctuations will not affect the accuracy of the final solution of our implementation by much.
...
To upsize/downsize a gate, we need to decrease/increase the slew target of its output pins, respectively. To facilitate MCMM optimization, each pin should have slew targets for all combinations of corners and delay modes. Now we need to address the following questions:
\begin{enumerate}
...
Let $slewt(p)$ be the slew target of an output pin $p$ of gate $g$. In setup time analysis, if $p$ has a negative slack and it is on a critical path, we upsize $g$ by decreasing $slewt(p)$; otherwise, we downsize $g$ by increasing $slewt(p)$. In hold time analysis, the operation is opposite: downsize the gate when one of its output pins has negative hold time slack and the pin is on a critical path, and upsize otherwise.

To know whether $p$ is on a critical path, we compute the local criticality of $p$, $lc(p)$, as follows. Let $slack^{-}(g) = \min\{slack(j) | (i, j) is a wire and j is g's input\}$. Then $lc(p) = \max\{slack(p) - slack^{-}(g), 0\}$. $p$ is on a critical path if $lc(p) = 0$.

Now we elaborate how to update the value for slew target of $p$. Table~\ref{table:lib1} and~\ref{table:lib2} show the rising slews for buffer X1 and buffer X2, respectively. Note that (1) both tables contain similar values, and (2) for a given slew and load, the corresponding entry for buffer X2 is left to the one for buffer X1, and the two entries are always on the same row. For example, for slew value of 15.6743 and load value of 30.3269, it corresponds to row 3 and column 6 for buffer X1, but row 3 and column 5 for buffer X2. Therefore, the slew target of $p$ can be updated according to a sequence of numbers derived through table look-up using current input slew and different output capacitance. We call this sequence the slew possibilities of $p$.

To upsize/downsize a gate, we take smaller/larger entries in $p$'s slew possibilities, which can be approximated as follows. Through table look-up, the smallest entry $lb$ is computed using the current input slew and zero driving capacitance, and the largest entry $ub$ is computed using the current input slew and the maximum driving capacitance of $p$. Intermediate entries can be approximated based on a geometric sequence whose common ratio $r = (\frac{ub}{lb})^{\frac{1}{k}})$ where $k$ is a tunable parameter. In this contest, we have 8 entries in slew possibilities, and we set $k = 20$. The intermediate 6 terms are $lb\cdot r$, $lb\cdot r^3$, $lb\cdot r^5$, $lb\cdot r^8$, $lb\cdot r^{11}$, and $lb\cdot r^{15}$.
...
Having the slew target updated, we now size the gate $g$. Since output capacitance usually has larger impact on the cell timing than the input slew, we want to fix all the downstream gates of $g$ before sizing $g$. This requires a reverse topological order of all gates. However, gates may form cycles in a synchronous design using registers. To resolve this issue, we ignore the edges feeding into register inputs to have a directed acyclic graph (DAG), from which we define a reverse topological order for gates.
\subsubsection{Input Slew Estimation}
As our delay model is NLDM, and upstream gates of $g$ are not fixed yet, we need to estimate slews for each of $g$'s input pins. Figure~\ref{fig:slew} shows an example. Let $est\_slew(p) = \theta slewt(p) - (1-\theta)slew(p)$ be the estimated slew at output pin $p$. Then, $est\_slew(q) = est\_slew(p') + slew\_degrad(p', q)$ where $slew\_degrad(p', q)$ denotes the slew degradation from $p'$ to $q$, which can be computed by the RC-tree model~\cite{TAU:Contest}.

\subsubsection{Cell Selection}
Let $out\_est\_slew(p)$ be the estimated output slew at pin $p$ through table look-up using load seen by $p$ and $est\_slew(q)$, where $(q, p)$ is a timing arc. A pin may have different slew targets for each combination of timing corners and setup/hold time analysis. 
\begin{itemize}
\item In setup time analysis, we select for gate $g$ the minimum size of possible cells such that $out\_est\_slew(p) \leq slewt(p)$ holds for all output pins $p$ of gate $g$ at the corner being considered. Then we select the maximum sized cell from the selected cells across all corners for gate $g$ in setup time analysis. We define $size_{s}(g)$ as the cell size selected for $g$ in setup time analysis.

\item In hold time analysis, the selection works in the opposite way: we select the maximum size of possible cells such that $out\_est\_slew(p) \geq slewt(p)$ holds for all output pins $p$ of gate $g$ at the corner being considered; and then select the minimum sized cell from the selected cells across all corners for gate $g$ in hold time analysis. We define $size_{h}(g)$ as the cell size selected for $g$ in hold time analysis.
...
\item If $size_{s}(g) > size_{h}(g)$, then assign the cell of size $size_{h}(g)$ to gate $g$. This is because there is no cell size that can satisfy both slew targets in setup time and hold time analysis at the same time. We honor hold time constraints in this case without sacrifice setup time seriously, since hold time violations cannot be fixed by adjusting the clock frequency.
\end{itemize}
